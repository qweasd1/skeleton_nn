...
root = "/Users/zhendongwang/Documents/projects/phd/skeleton_nn/code/nn_mnist/models"
data_root = "/Users/zhendongwang/Documents/projects/phd/skeleton_nn/code/nn_mnist/data"
...
class EnsembleModel:
    ...
    def predict(self, X):
        result = torch.zeros((len(X),)).fill_(-1).to(device)
        all_prob = torch.zeros((len(X),)).fill_(-float("Inf")).to(device)
        candidates = {}

        for target, binary_model in self.models.items():
            pred, prob = binary_model.predict(X)
            for i in pred.nonzero().view(-1):
                if prob[i] > all_prob[i]:
                    result[i] = target
                    all_prob[i] = prob[i]
                if i.item() not in candidates:
                    candidates[i.item()] = []
                candidates[i.item()].append((target, prob[i].item()))
        for minus1_index in (result == -1).nonzero().view(-1):
            _candidates = [x for x in self.predict_single(X,minus1_index).items()]
            _candidates.sort(key=lambda x: x[1], reverse=True)
            result[minus1_index] = _candidates[0][0]

        return result, candidates

    def predict_single(self, Xt, index):
        xt = Xt[index].view(1, 1, 28, 28)
        result = {}

        for target, binary_model in self.models.items():
            for subsubmodel in binary_model.models:
                y_pred = subsubmodel(xt)

                if target not in result:
                    result[target] = y_pred[0,1].item()
                else:
                    if result[target] < y_pred[0,1].item():
                        result[target] =  y_pred[0,1].item()
        return result
...
if __name__ == '__main__':
    ...
    # _Xt = torch.load("{0}/Xt.pt".format(data_root)).to(device)
    ...
    Xt = torch.load("{0}/Xt.pt".format(data_root)).to(device)
    yt = torch.load("{0}/yt.pt".format(data_root)).to(device)

    # Ensemble model
    model = EnsembleModel(device)
    model.load_binary_models(root)

    y_p, candidates = model.predict(Xt)
    print("acc: {0}".format((y_p == yt).sum().item() / len(y_p)))

    # all in one model
    # model = CNNModel(10)
    # model.load_state_dict(torch.load("/Users/zhendongwang/Documents/projects/phd/skeleton_nn/code/nn_mnist/all_model/all_model"))
    # model.eval()
    #
